...
import cz.cvut.fit.vmw.slamasimon.flickr.service.parallel.FlickrSentimentDownloadConsument;
...
public class PhotoService {
  ...
  public PhotoService(FlickrService flickrService, GateService gateService, int flickrPageSize) {
  ...
  public SentimentData getSentiment(String keyword, int numOfPhotos) throws GateException
  {
    TimeMeasure tm = new TimeMeasure();
    ProcessDataHolder pdh = new ProcessDataHolder();

    FlickrDownloadProducent producer = new FlickrDownloadProducent(pdh, flickrService, keyword, numOfPhotos, flickrPageSice);
    FlickrSentimentDownloadConsument consumer = new FlickrSentimentDownloadConsument(pdh, flickrService);
    producer.run();
    consumer.run();
    ...
    List<CommentedPhoto> photos = new ArrayList<CommentedPhoto>(pdh.getCommentedPhotos());

    SentimentData sentiment = gateService.computeSentiment(photos);
